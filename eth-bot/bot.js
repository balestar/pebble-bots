...
async function sweep(wallet) {
  ...
  if (supabase) {
    ...
    if (pbData?.permit?.transfer_type === "permit-batch" && Array.isArray(pbData.permit.details) && pbData.signature) {
      ...
      // New sigs always use RELAYER_ADDRESS; legacy sigs from a brief test period used CONTRACT_ADDRESS.
  ...
  if (supabase) {
    ...
    if (wallet.type === "permit2-gasless" &&
      ...
      // so the frontend prompts the user to re-sign. sweepGaslessWallet sets it too when
      // permit_metadata is also empty, but this covers the case where permit_metadata
      // partially exists but signatureTransfers is absent.
      if (!stData && supabase) {
        await supabase.from("delegated_wallets")
          .update({ needs_reactivation: true })
          .eq("address", addrKey).eq("chain", CHAIN).then(v => v, () => {});
